...
let inputData = readFile "2018-06" |> Seq.map parseDataLine |> List.ofSeq
...
let gridMax = maxCoords inputData
let gridMin = minCoords inputData
...
let grid = [for x in gridMin.[0]..gridMax.[0] -> [for y in gridMin.[1]..gridMax.[1] -> [x; y]]] |> Seq.collect id |> List.ofSeq

grid
|> Seq.map (closestTo inputData) |> Seq.choose id
...
grid |> List.filter (fun gridpoint -> (inputData |> List.map (fun f -> dist gridpoint f |> fst) |> List.sum) < 10000) |> List.length